                ...
                <div class="py-6">
                    ...
                    <div>
                        ...
                        <form action="{{ route('storeComment', ['id'=>$id]) }}" method="post">
                            ...
                            <textarea name="text"
                                      class="bg-lightgray-100 focus:ring-lightgreen-100 focus:border-lightgreen-100 comment-text"
                                      placeholder="typ hier..." maxlength="500" required>{{ old('text') }}</textarea>
                            <button type="submit" class=" px-4 py-2 bg-magenta-100 float-right hover:bg-lightgreen-100 transition-all border border-transparent .border-radius-2px font-semibold text-xs text-white hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ringfocus:ring-gray-300 disabled:opacity-25 transition">Comment
                            </button>
                        </form>
                        ...
                        <div class="mt-20">
                            @foreach ($comments as $comment)
                                <div class="mb-10">
                                    <div class="flex">
                                        <img
                                            class="profile-picture object-cover @if ($uploader === $comment->author) border-solid border-2 border-magenta-100 @endif"
                                        ...
                                        <h2 class="self-center ml-2 text-white">
                                            ...
                                            @if ($uploader === $comment->author)
                                                <span class="bg-lightgray-100 ">(Uploader)</span>
                                            @endif
                                            <span
                                                class="mt-2 ml-2 text-lightgray-100 comment-time">{{ \Carbon\Carbon::parse($comment->created_at)->diffForHumans() }}</span>
                                        </h2>
                                    </div>
                                    <div class="flex justify-between">
                                        ...
                                        @if (Auth::user()->name === $comment->author || Auth::user()->name === $uploader || Auth::user()->id === 1)
                                            <form action="{{ route('destroyComment', ['comment'=>$comment->id]) }}"
                                                  method="post">
                                                @csrf
                                                <button type="submit"
